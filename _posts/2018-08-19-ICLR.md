...
on top by a large margin, far ahead of
alternatives like Adagrad, RMSProp or SGD (we'll come to
...
How have we reached this point, where the most widely used
...
Let's define our online optimization framework.
At each time step $t$:
...
issues, **adaptive methods** build upon the key idea that **each weight, that
is each coordinate of $x_t$, should be updated using its own learning rate**.
...
We call $\alpha_t$ the step size, and $\alpha_t/\sqrt{V_t}$ the learning rate.
Note that, using only diagonal variants $V_t = \text{diag}(v_t)$, this is
essentially saying that the step size for a given parameter is scaled by some
function of past gradients *of this specific parameter*. Within this framework,
we can see the evolution of adaptive methods in time:

1. **(SGD) Stochastic gradient descent**
   <center>$\phi_t(g_1, ..., g_t) = g_t$ and
   $\psi_t(g_1, ..., g_t) = \mathbb{I}$</center>
   SGD forgets everything about the past of each weight, and relies only on the
   current gradient to perform the update, with a step size
   (and learning rate) $\alpha_t = \alpha/\sqrt{t}$, an aggressive decay.<br><br>

2. **(AdaGrad) Adaptive gradient descent**
   <center>$\phi_t(g_1, ..., g_t) = g_t$ and
   $\psi_t(g_1, ..., g_t) = {\text{diag}(\frac{1}{t} \sum_{i=1}^t g_i^2)}$</center>
   ...
   parameter $j$ is not frequently updated, this **considerably speeds up
   convergence** (updates are still rare, but much bigger than with SGD).<br><br>

3. **(RMSProp)**
   <center>$\phi_t(g_1, ..., g_t) = g_t$ and
   $\psi_t(g_1, ..., g_t) = {\text{diag}((1-\beta)\sum_{i=1}^t \beta^{t-i} g_i^2)}$</center>
   RMSProp first considered restricting the average of past gradients to
   a fixed window, instead of the entire past. In practice, this is implemented
   by an exponentially moving average of past gradients.<br><br>

4. **(Adam) Adaptive momentum estimation**
   <center>$\phi_t(g_1, ..., g_t) = (1-\beta_1)\sum_{i=1}^t \beta_1^{t-i} g_i^2{\ }$ and
   $\ \psi_t(g_1, ..., g_t) = {\text{diag}((1-\beta_2)\sum_{i=1}^t \beta_2^{t-i} g_i^2)}$</center>
   Adam adds to RMSProp the idea of momentum (see Ruder's post) to further

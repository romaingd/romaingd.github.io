...
$x = +1$. The intuition for this behavior is the following: although the large
gradient $C$ is observed every $3$ steps, it is **forgotten (scaled down)
too quickly** (by the exponential weights) to counterbalance the wrong but most
frequent updates.
